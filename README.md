...
- Enable **Particle mist**, then use the **0–100%** density slider. It adds translucent clouds in the 3D scene around each race camera in GP and VS. Zero adds no mist; normal course fog is preserved. The effect uses native particles, which can intersect scenery and cost performance in split-screen.
...
- Mist and speed start disabled, preserving normal game settings. Moving either slider updates an already-generated code, including in per-position mode. Copy the replacement code and fully restart emulation; do not use an old save state. The small mist preview is illustrative, not a game screenshot.
...
**Particle mist** was visually tested in a live single-player GP race in an isolated Dolphin profile. Native hook tests cover GP/VS, camera counts, particle-allocation failure, null-pointer guards and register preservation. Portable checks cover all 101 density settings, all 9,851 speed settings and combinations with every existing option, including kart size. The speed initializer retains its 147,765-case native regression suite. Replace any previous fog code and fully restart Dolphin without loading a save state. See [mist and speed patch notes](docs/fog-speed.md).
